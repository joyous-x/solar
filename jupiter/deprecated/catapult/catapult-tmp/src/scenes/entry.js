...
export default {
  init() {
    ...
    const levelLocker = [0, 25, 50, 0, 0]
    !['blue', 'pink','yellow', /* 'chart', 'custom', */ 'setting'].forEach((id, index) => {
      const statusLocked = store.unlocked < levelLocker[index]
      const resName = (statusLocked !== true) ? `circle.${id}.png` : `circle.lock.png`
      const item = PIXI.Sprite.from(resName)
      item.name = id
      item.locked = statusLocked
      item.locker = levelLocker[index]
      item.index = index
  ...
  listen() {
    ...
    container
      ...
      .on('tap', ev => {
        ...
        if (target.locked || !target.name || target.parent !== this.layout) {
            if (target.locked && target.name) {
                const describtion = `${target.name} 需要 ${target.locker} 关才能解锁`
                platform.showToast({
                    title: describtion,
                    icon: 'none',
                    duration: 3e3
